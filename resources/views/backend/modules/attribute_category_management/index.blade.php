                ...
                <div class="col-lg-6 col-7">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                        <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Attribute Category Management</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-lg-6 col-5 text-right">
                    <a class="waves-effect waves-light btn btn-sm btn-neutral modal-trigger" data-toggle="modal"
                        data-content="{{ route('attribute-category.create') }}" href="#modal1">Add AttributeCategory</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class=" col ">
            <div class="card">
                <div class="card-header bg-transparent">
                    <h3 class="mb-0">All AttributeCategory List</h3>
                </div>
                <div class="card-body">
                    <div class="row icon-examples">
                        <div class="table-responsive col-md-12">
                            <table class="table table-bordered attribute-category-datatable" id="datatable">
                                <thead>
                                    <th>ID</th>
                                    <th>AttributeName</th>
                                    <th>CategoryName</th>
                                ...
                                <tbody>
                                    @foreach($attribute_categories as $att_cat)
                                    <tr>
                                        <td>{{ $att_cat->id }}</td>
                                        <td>{{ $att_cat->attribute_id }}</td>
                                        <td>{{ $att_cat->category_id }}</td>
                                        <td>
                                            @if($att_cat->is_active == 1)
